...
const prompt = ChatPromptTemplate.fromMessages([
  ["system", "You are a helpful assistant generating output strictly in Markdown format. Use the provided context to answer."],
  ["assistant", "Context:\n {context}"],
  ["human", "{input}"]
]);
...
export const chatHandler = async (req, res) => {
  try {
    ...
    contexts.forEach(context => {
      if (context[0]?.pageContent && typeof context[0].pageContent === "string") {
